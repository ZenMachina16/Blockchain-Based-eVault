...
async function main() {
  ...
  const addressToAddClient = await getUserInput(
    "Enter the address you want to add as a client: ",
  );
  ...
  const isCourtOfficial = await NavinEvault.isCourtOfficial(
    addressToAddClient.trim(),
  );
  if (isCourtOfficial) {
    console.error(
      `Address ${addressToAddClient.trim()} is already a court official and cannot be a client.`,
    );
