...
async function main() {
  ...
  const addressToMakeOfficial = await getUserInput(
    "Enter the address you want to make a court official: ",
  );

  // Validate address format using regex
  const isValidAddress = /^0x[a-fA-F0-9]{40}$/.test(
    addressToMakeOfficial.trim(),
  );
  if (!isValidAddress) {
    console.error(
  ...
  const NavinEvault = await hre.ethers.getContractAt(
    "NavinEvault",
    contractAddress,
    owner,
  );
  ...
  const isAlreadyOfficial = await NavinEvault.isCourtOfficial(
    addressToMakeOfficial.trim(),
  );
  if (isAlreadyOfficial) {
    console.log(
      `Address ${addressToMakeOfficial.trim()} is already a court official.`,
    );
  ...
  if (isClient) {
    console.error(
      `Address ${addressToMakeOfficial.trim()} is already a client and cannot be made a court official.`,
    );
  ...
  console.log(
    `Successfully added court official: ${addressToMakeOfficial.trim()}`,
  );
}
